...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{CDC Projet Génie logiciel 2012-2013}
...
Nous envisageons d'ajouter aux informations stockées les \glspl{metadonnee} propres aux fichiers\index{Fichier}, réguliers, audio et vidéo afin que l'utilisateur\index{Utilisateur} puisse faire une recherche sur celles-ci (par exemple sur l'auteur d'une chanson).
...
Il s'agit là d'un programme qui doit être lancé lors du démarrage de la session de l'utilisateur\index{Utilisateur}, autrement dit un \gls{daemon}\index{Daemon}. Une connexion avec la base d'indexation\index{Base d'indexation} va alors être ouverte afin de permettre l'envoie de message.
...
Pour chaque modification\index{Modification}, un flux \gls{xml}\index{XML} (respectant la \gls{dtd}\index{DTD} correspondante, p.~\pageref{dtd_bi_mi}) va alors être envoyé à la base d'indexation\index{Base d'indexation}.
...
En cas de changement d'\gls{arborescence}\index{Arborescence}, une demande de suppression\index{Suppression} de tous les \glspl{fichier}\index{Fichier} actuellement surveillés est alors demandée.
...
\end{itemize}
Pour ces trois cas, la modification\index{Modification} sur la base de donnée\index{Base de donn\'{e}es} doit se faire de manière complète, c'est à dire sur toutes les entrées correspondantes dans chacune des bases concernées. Par exemple, la transformation d'un fichier\index{Fichier}, régulier, audio en fichier\index{Fichier} texte, doit non seulement mettre à jour les informations dans la table \enquote{AnyFile} mais également la ligne correspondant au fichier\index{Fichier} dans la table Audio.
...
Le base d'indexation\index{Base d'indexation} ne doit en aucun cas se déconnecter sans avoir répondu à toutes les requêtes qu'elle a reçu.\\
Il doit pouvoir gérer les requêtes des moteur de recherche\index{Moteur de recherche} et d'indexation\index{Moteur d'indexation} ne respectant pas la \gls{dtd}\index{DTD}. Toutes les requêtes crées par le moteur de recherche\index{Moteur de recherche} doivent être conforment à la \gls{dtd}\index{DTD}.
